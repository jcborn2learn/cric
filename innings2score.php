...
<div id="main-wrapper">
<form class="myform" action="innings2score.php" method="post">
<center>
<label>Striker:</label>
...
if(isset($_POST['run'])){
	...
	if(isset($_POST['wide']) && !isset($_POST['noball']) && !isset($_POST['wicket'])){
		...
		$query_run=mysqli_query($con,$query);
		extrainsert($con);
		$teamtotal=teamtotalscore($con);
		$_SESSION['teamtotal']=$teamtotal;
		echo "totalteam score is:"; echo $teamtotal;
	}
	if(!isset($_POST['wide']) && isset($_POST['noball']) && !isset($_POST['wicket'])){
	    $variable_name=mysqli_real_escape_string($con,$_POST['run']);  
	    $val=$variable_name+1;
...
if(mysqli_num_rows($query_run)>0){
	...
	$oversmatch=mysqli_real_escape_string($con,$_SESSION['overs']);
	$k=($oversmatch)*6-$fromballs;
	$t=mysqli_real_escape_string($con,$_SESSION['target']);
	$p=$t-$teamtotal;
    echo "Need "; echo $p; echo " from "; echo $k; echo " balls"; echo "<br></br>";
if($p<=0){
    echo '<script type="text/javascript"> alert("Second batted team won the match..Enter Innings over.")</script>';
}
else{
}
	...
	if($value==6){
		...
		if($noofovers==$_SESSION['overs']){
			echo '<script type="text/javascript"> alert("First batted team won the match..Enter Innings over.")</script>';
		}
		else{
...
if(isset($_POST['run'])){
	...
	$table_name2=mysqli_real_escape_string($con,$_SESSION['nonstrikername']);
	...
	$query="select count(*) from $table_name2 where score='4'";
	$query_run=mysqli_query($con,$query);
	if(mysqli_num_rows($query_run)>0){
		while($row=mysqli_fetch_array($query_run)){
			$result2=$row['count(*)'];
	...
	$query="select count(*) from $table_name2 where score='6'";
	$query_run=mysqli_query($con,$query);
	if(mysqli_num_rows($query_run)>0){
		while($row=mysqli_fetch_array($query_run)){
			$result3=$row['count(*)'];
    ...
    $query="select sum(score) from $table_name2";
	$query_run=mysqli_query($con,$query);
	if(mysqli_num_rows($query_run)>0){
		while($row=mysqli_fetch_array($query_run)){
			$result4=$row['sum(score)'];
	...
	$table_name=mysqli_real_escape_string($con,$_SESSION['nonstrikername']);
	$query="update batting set runs='$result4',balls='$result1',fours='$result2',sixes='$result3' where batsname='$table_name2'";               
	$query_run=mysqli_query($con,$query);
	
	echo "<table border='1'>";
	echo "<tr><td> batsman </td><td> R </td><td> B </td><td> 4s </td><td> 6s </td></tr>";
	echo "<tr><td>".$table_name2."</td><td>".$result4."</td><td>".$result1."</td><td>".$result2."</td><td>".$result3."</td></tr>";
	echo "</table>";
}
?>
